...
const lifecycle = [
  {
    ...
    buyer: 'Give the agent permission for one job — not a blank check.',
    technical: 'Mint a macaroon-style capability with caveats for tenant, task, allowed tools, budget, expiry, and delegation depth.',
    status: 'Allowed',
  ...
  },
  {
    ...
    buyer: 'Let a worker help, but keep it inside the parent’s lane.',
    technical: 'Create a child token with parent_token_id, depth=1, child spend cap, subset tool scope, and shorter TTL.',
    status: 'Allowed',
  ...
  },
  {
    ...
    buyer: 'The child gets less authority than the parent: fewer tools, less money, shorter time.',
    technical: 'Apply caveats: max_depth=1, child_budget_usd=5, tools=[repo_search], denied_tools=[payment_release], ttl=15m.',
    status: 'Constrained',
  ...
  },
  {
    ...
    buyer: 'If the job is done, the agent loops, or the parent is revoked, the child is denied on its next gateway-enforced request.',
    technical: 'Set revocation_state=revoked for the child token; in a governed deployment, reject the next request at the gateway before upstream execution.',
    status: 'Denied',
  ...
  },
  {
    ...
    buyer: 'Security, finance, and compliance get the receipt: who authorized what, which limits applied, why it was blocked, and what spend remained.',
    technical: 'Demo Evidence Pack includes parent/child lineage, caveats, policy version, decision, remaining budget, revocation state, and receipt IDs.',
    status: 'Proved',
...
const caveats = [
  ...
  { label: 'Child spend cap', value: '$5.00', buyer: 'worker cannot burn the parent budget' },
  { label: 'Child spend used', value: '$0.28 / $5.00', buyer: 'spend math stays visible' },
  { label: 'Delegation depth', value: '1 child level', buyer: 'limits governed sub-agent delegation chains' },
  { label: 'Allowed child tools', value: 'repo_search, invoice_read', buyer: 'read-only helper tools' },
  { label: 'Denied tools', value: 'payment_release, vendor_update', buyer: 'high-risk actions blocked' },
...
const buyerTranslations = [
  ['Macaroon', 'A portable capability: a portable credential carrying bounded-authority caveats.'],
  ['Caveat', 'A business limit: budget, tool, route, tenant, time, delegation depth, or revocation rule.'],
  ['Attenuation', 'Making a child token narrower than the parent. Delegation adds caveats and should not remove or widen parent caveats; gateway policy enforces the semantics.'],
  ['Delegation depth', 'How many handoffs are allowed before the chain must stop.'],
  ['Revocation check', 'A kill switch evaluated before the next model, API, or MCP tool call.'],
  ['Evidence Pack', 'An audit bundle showing who authorized what, which limits were evaluated, and why SatGate allowed or denied the demo request.'],
];
    ...
    <main className="min-h-screen bg-black text-gray-100 font-sans">
      <section className="relative overflow-hidden border-b border-gray-900">
        ...
        <div className="relative mx-auto max-w-6xl px-6 py-20 md:py-24">
          <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-950/30 px-4 py-2 text-sm text-cyan-200">
            <ShieldCheck size={16} /> Capability lifecycle demo
          </div>
          <h1 className="mb-8 max-w-5xl text-5xl font-extrabold tracking-tight md:text-7xl">
            Show the full authority lifecycle: issue, delegate, attenuate, revoke, prove
          </h1>
          <p className="mb-10 max-w-4xl text-xl leading-relaxed text-gray-300 md:text-2xl">
            SatGate turns macaroons and caveats into buyer-visible controls: scoped authority, child spend caps, delegation depth, next-request revocation, and Evidence Pack-style audit records before governed access to paid APIs, models, or MCP tools. Observe who is acting, Control what can happen, and Prove the decision trail.
          </p>
      ...
      <section className="border-y border-gray-900 bg-gray-950/60">
        <div className="mx-auto max-w-6xl px-6 py-20">
          <h2 className="mb-4 text-3xl font-bold text-white">Macaroons and caveats, translated for buyers</h2>
          <p className="mb-10 max-w-4xl text-lg leading-relaxed text-gray-400">
            Engineers can talk about attenuation. Buyers need to see bounded authority: what the agent may do, how far it may delegate, how much the child may spend, when it dies, and what proof remains.
          </p>
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {buyerTranslations.map(([term, translation]) => (
        ...
        <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            ...
            <p className="mb-6 text-lg leading-relaxed text-gray-400">
              A credible lifecycle demo does not stop at “token issued.” It shows the child token’s narrower caveats, visible depth limit, next-request revocation result, and receipt context.
            </p>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24 pt-4">
        <div className="rounded-3xl border border-purple-900/60 bg-gradient-to-br from-purple-950/30 to-cyan-950/25 p-8 md:p-12">
          <h2 className="mb-4 text-3xl font-bold text-white">This is the capability story buyers need to see.</h2>
          <p className="mb-8 max-w-4xl text-lg leading-relaxed text-gray-300">
            Issue the capability, delegate a narrower child, show the caveats in plain English, revoke before the next governed request, and export the proof. That is how macaroon-style caveats become enterprise-readable agent authority controls.
          </p>
          <div className="flex flex-col gap-4 sm:flex-row">
